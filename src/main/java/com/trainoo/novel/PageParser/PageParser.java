...
public class PageParser {
	...
	public PageParser(File file, int currentpage) {
		try {
	...
	private String read() {
		...
		try {
			while(null != readFile.readLine() && !readFile.readLine().contains(name)){
	...
	public String getPre() {
		...
		if (currentpage <= 1) {
			...
			// 其它页 则定位到当前页-2 处 在读取指定字节内容 例如当前定位到第二页的末尾 2*SIZE 上一页应该是第一页 也就是从0位置
			// 开始读取SIZE个字节
			seek(currentpage - 2);
			content = read();
